...
import { ApiPagingRequest, DialogProps, RolesDataGirdProps, RolesProps, SortProps } from '../../../data-type'
import { ApiResponseBasic, ApiResponsePagingProps } from '../../../data-type/apiResponse/apiResponseProps'
import { DeleteRoleAsync, GetRolesPaging } from '../../../services'
...
const Roles = () => {
  ...
  const [rolesData, setRolesData] = useState<RolesProps[]>([])
  ...
  const [rowsSelected, setRowsSelected] = useState<RolesProps[]>([])
  ...
  const ButtonCreate = withComponentPermissionModule(Button, ModuleConfig.ROLES, [PermissionConfig.Create])
  const MenuItemCreate = withComponentPermissionModule(MenuItem, ModuleConfig.ROLES, [PermissionConfig.Create])
  const IconButtonDelete = withComponentPermissionModule(MenuItem, ModuleConfig.ROLES, [PermissionConfig.Delete])

  const pageSize = 10

  const columns: GridColDef[] = [
    { field: 'code', headerName: t('roles:dataGrid.headers.code'), width: 130 },
    { field: 'name', headerName: t('roles:dataGrid.headers.name'), width: 130 },
    {
      field: 'isActive',
      headerName: t('roles:dataGrid.headers.status'),
      sortable: false,
      width: 160,
      valueGetter: (_value, row) => `${row?.isActive ? t('common:active') : t('common:deActive')}`,
    },
    { field: 'description', headerName: t('roles:dataGrid.headers.description'), width: 130, sortable: false },
    {
      field: 'actions',
      type: 'actions',
      headerName: t('roles:dataGrid.headers.actions'),
      width: 100,
  ...
  const onDelete = async () => {
    ...
    onLoading(true)
    await DeleteRoleAsync<ApiResponseBasic>({ Ids: ids as number[] })
      .then(res => {
        WriteLogTable({
          data: res,
  ...
  const onRowsSelectionHandler = (ids: GridRowSelectionModel) => {
    ...
    if (rolesData && rolesData.length !== 0) {
      const selectedRows = rolesData.filter((row: RolesProps) => selectedIDs.has(row?.id as number))
  ...
  const renderRemoveButton = () => {
    if (rowsSelected && rowsSelected.length !== 0) {
      return (
        <Button variant='outlined' size='small' onClick={handleDeleteAndClose}>
          <DeleteIcon />
  ...
  const renderMenuItemRemoveButton = () => {
    if (rowsSelected && rowsSelected.length !== 0) {
      return (
        <IconButtonDelete onClick={handleDeleteAndClose}>
          <DeleteIcon sx={{ mr: 1 }} />
          {t('common:actions.delete')}
        </IconButtonDelete>
      )
    ...
    return (
      <Fragment>
        <Stack sx={{ display: { xs: 'none', md: 'flex' }, mr: 1 }} direction='row' spacing={1} mb={2} divider={<Divider orientation='vertical' flexItem />}>
          ...
          <Box sx={{ display: 'flex' }}>
            <OutlinedInput sx={{ ml: 1, width: '40ch' }} size='small' placeholder={t('roles:searchPlaceholder')} value={keywordValue} onChange={e => setKeywordValue(e.target.value)} />
        ...
        <Stack sx={{ flexGrow: 1, display: { xs: 'flex', md: 'none' } }} direction='row' spacing={1} mb={2}>
          ...
          >
            ...
            <MenuItem>
              <OutlinedInput sx={{ ml: 1 }} size='small' placeholder={t('roles:searchPlaceholder')} value={keywordValue} onChange={e => setKeywordValue(e.target.value)} />
              <IconButton onClick={handleSearch} type='button' sx={{ p: '10px' }} aria-label='search'>
                <SearchIcon />
              </IconButton>
            </MenuItem>
  ...
  const fetchData = async () => {
    ...
    setLoading(true)
    await GetRolesPaging(params)
      .then(res => {
        const items: ApiResponsePagingProps<RolesProps[]> = res as ApiResponsePagingProps<RolesProps[]>
        if (items?.status) {
          setTotalPage(items?.data?.totalPages ?? 1)
          setTotalCount(items?.data?.totalCounts ?? 1)
          if (items?.data && items?.data?.records?.length !== 0) {
            items?.data?.records?.map((item: RolesDataGirdProps) => {
              rows.push({
                id: item?.id,
                code: item?.code,
                name: item?.name,
            ...
            WriteLogTable({
              ...
              groupTitle: 'Response from API GetRolesPaging',
            })
            setRolesData(rows)
          } else {
            setRolesData([])
  ...
  return (
    <Box component='section'>
      <Breadcrumbs aria-label='breadcrumb' sx={{ mb: 2 }}>
        ...
        <Typography color='text.primary'>{t('common:menuTop.roles')}</Typography>
      </Breadcrumbs>
